...
| 14 | 부분 충족 | API Markdown 0.3.0, 최신 ERD, 분석 이력 Endpoint를 반영했다. `docs/api/openapi.yaml`은 아직 없어 구조 자동 대조는 미검증이다. |

현재 결론은 **4개 충족, 8개 부분 충족, 2개 미충족**이다. 스키마 단독으로 처리할 수 없는 항목을 완료로 표시하지 않는다.
