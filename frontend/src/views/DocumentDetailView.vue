...
<template>
  <div class="page">
    ...
    >
      ...
      <template #subject>{{ document?.title ?? '문서' }}</template>
      <template #actions>
        <RouterLink :to="{ name: 'document-preview', params: { documentId: String(documentId) } }">
          <PillButton variant="quiet">Preview 열기</PillButton>
        </RouterLink>
      </template>
    </PageHero>
...
.rrow:hover .arrow { transform: translateX(3px); stroke: var(--primary-600); }
...
.arrow { justify-self: end; margin-top: 3px; transition: transform .3s var(--ease); }
