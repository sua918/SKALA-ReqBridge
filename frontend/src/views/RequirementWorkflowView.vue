...
<template>
  <div class="page">
    ...
    >
      <template #title>요구사항 검토</template>
      <!-- 상태 배지는 #actions(버튼 자리)가 아니라 제목 옆에 둔다.
           그 자리는 「누르는 것」 자리이고, 배지는 못 누른다. 그래픽 위에 얹히면
           바탕이 계속 변해 읽히는 정도도 일정하지 않다. -->
      <template #subject>
        <span class="subjline">
          <!-- 순번을 못 읽었을 때 저장번호로 대신하면, 같은 자리에 뜻이 다른 두 수가
               번갈아 뜬다. 값이 없으면 순번 자리를 비운다. -->
          <template v-if="requirement?.sequenceNo != null">#{{ requirement.sequenceNo }}</template>
    ...
    <div v-else-if="workflow" class="grid12 split at-4">
      ...
      <div class="right" data-reveal>
        ...
        <div v-if="proposedRevision" class="card pad revision live">
          <div class="cardhead">
            <span class="eb">{{ proposedRevision.revisionNo }}차 수정안</span>
            <StatusBadge kind="revision" :value="proposedRevision.status" />
          </div>
          <p class="revtext">{{ proposedRevision.text }}</p>
          <!-- 어떤 답변을 반영했는지는 위 문제·질문 목록에 그대로 있다.
               여기에 질문 ID를 늘어놓아도 짚어갈 곳이 없다. -->
          <p v-if="proposedRevision.basedOnClarificationIds.length" class="mi hint">
            근거 답변 {{ proposedRevision.basedOnClarificationIds.length }}건
        ...
        <div v-for="revision in reviewedRevisions" :key="revision.id" class="card pad revision past">
          <div class="cardhead">
            <span class="eb">{{ revision.revisionNo }}차 수정안</span>
